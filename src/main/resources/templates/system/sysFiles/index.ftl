...
<@mc.ltfooter  type="list" import="paginator">
    ...
    <script type="text/javascript">
        ...
        function initTable(type, page, obj) {
            ...
            param.fileType = queryType == "all" ? '' : queryType;
        ...
        function renderTable(result, page) {
            ...
            $.each(data, function (i, file) {
                ...
                if (isImage(file.fileType)) {
                    var url = prefix + '/loadFile?file=' + file.location;
                    fileIcon = '<div style="cursor:pointer;" title="' + this.originName + '" class="image fancybox" href="' + url + '"><img alt="image" style="width:100%;height:100%;" class="img-responsive" src="' + url + '"></div>';
                }
                fileHtml += '<div class="file-box">' +
                    '    <div class="file" title="' + this.originName + '">' +
                    '        <span class="corner"></span>' + fileIcon +
                    '        <div class="file-name">' +
                    '            <span>' + this.originName + '</span><br/>' +
                    '            <small>添加时间：' + file.createTime + '</small><br/>' +
                    '<@shiro.hasPermission name="funFile-btnDelete"><a class="m-l-lg" onclick="fileDelete(\'' + file.id + '\')"><i class="fa fa-remove"></i>删除</a></@shiro.hasPermission>' +
                    '<@shiro.hasPermission name="funFile-btnDownload"><a class="m-l-lg" onclick="fileDownload(\'' + file.id + '\')"><i class="fa fa-cloud-download"></i>下载</a></@shiro.hasPermission>' +
                    '        </div>' +
                    '    </div>' +
                    '</div>';
            });
        ...
        function fileDownload(id) {
            lt.confirm("确定要下载选中文件？", function () {
                var url = prefix + "/downloadById?fileId=" + id;
                window.open(url);
            });
        }
        ...
        function fileDelete(id) {
            lt.confirm("确定要删除选中文件？", function () {
                $.post(prefix + '/delete', {id: id}, function (result) {
                    if (result.code == 200) {
