...
<@mc.ltfooter  type="list">
    <script type="text/javascript">
        ...
        function initTable() {
            lt.dataTable.init("#dataTable", {
                ...
                columns: [{field: 'ck', checkbox: true},
                    {
                        title: '系统编号', field: 'code',
                        formatter: function (value, row, index) {
                            return '<a onclick="view(\'' + row.id + '\')">' + value + '</a>';
                        }
                    },
                    {title: '系统名称', field: 'name'},
                    {
                        title: '系统类型', field: 'type',
                        formatter: function (value, row, index) {
                            return top.lt.cache.dictItems["systemType"][value];
                        }
                    },
                    {title: '当前版本', field: 'version'},
                    {title: '访问密码', field: 'secret'},
                    {title: '建设单位', field: 'devOrg'},
                    {title: '建设时间', field: 'devTime'},
                    {
                        title: "操作", field: "id",
                        formatter: function (value, row) {
                            ...
                            var v = '<@shiro.hasPermission name="funcSystem-btnVersionManage"><a class="btn btn-info btn-xs m-r-xs" onclick="version(\'' + row.code + '\');return false;"><i class="fa fa-code-fork"></i>版本管理</a></@shiro.hasPermission>';
                            return e + d + v;
                        }
                    }
                ]
        ...
        function version(code) {
            lt.open("系统版本管理", "/systemVersion/index/" + code);
        }
    </script>
</@mc.ltfooter>
</body>
